...
class OpenLibrarySearch {
  ...
  constructor(args: string[]) {
    ...
    if(this.author == undefined) { // if no cli argument inputted
  ...
  async search(): Promise<openLibraryReturnData[]> {
    return fetch(`http://openlibrary.org/search.json?author=${this.author}`) // fetch API data
      ...
      .then((json) => { // handle json data
        ...
        for (var i = 0; i < data.length; i++) { // loop over data
          let newData = { // create a new object with wanted data
